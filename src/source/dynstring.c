...
string* string_clone(string* s) {
    ...
    string* clone = new_string_with_size(s->capacity);
...
int32_t string_shrink(string* s) {
    if (!s) return 1;
...
int32_t string_copy(string* dest, string* src) {
    if (!dest || !src) return 1;
    if (dest->length > src->length)
        if (string_grow(dest, src->length));
            return 2;
...
int32_t string_push(string* s, const char* str) {
    if (!s || !str) return 1;
...
int32_t string_insert(string* s, size_t index, const char* str) {
    if (!s || !str || index < 0) return 1;
